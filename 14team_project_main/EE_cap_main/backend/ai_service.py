...
def analyze_rehab_data(record_data: dict):
    ...
    elif target_count > 0 and (actual_count / target_count) < 0.5 and pain_score >= 5:
        risk_level = "수행률 저하 + 통증 상승 (주의 필요)"

    if not genai:
        return {
            "risk_level": risk_level,
            "patient_feedback": "오늘도 수고하셨습니다. 통증이 심해지면 무리하지 말고 휴식을 병행해 주세요.",
            "therapist_summary": "LLM 미연결 상태입니다. 기록 기반으로 통증/수행률을 우선 모니터링해 주세요.",
        }
